...
import { fetchJson } from "@/app/lib/apiClient";
...
export default function SocialClient() {
  ...
  useEffect(() => {
    async function fetchSocialData() {
      ...
      if (!sectionsResponse.ok) {
        console.error("Sections error:", sectionsBody?.error || "Could not load sections.");
        setSections([]);
      ...
      if (!itemsResponse.ok) {
        console.error("Items error:", itemsBody?.error || "Could not load items.");
        setSections([]);
        setEventsLoading(false);
        return;
      }
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen gap-6 py-8 pb-24 px-4 bg-gradient-to-br from-purple-900 via-indigo-900 to-blue-900">
      <Link
        href={`http://localhost:3001/questlistpage${queryString}`}
